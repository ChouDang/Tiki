...
export default function Page(props: any) {
  ...
  useEffect(() => {
    const totalLogs = 6;
    const intervalTime = 10000 / totalLogs; // 10s / 6 = ~1.67s

    const interval = setInterval(() => {
      setCount((prevCount) => {
        if (prevCount >= totalLogs) {
          clearInterval(interval); // Dừng sau 6 lần log
          return prevCount;
        }
        return prevCount + 1;
      });
            ...
            items={[
              {
                ...
                children: (
                  <div className='w-[30rem]'>
                    <div className={`text-lg font-semibold ${count == 0 ? 'text-green-500' : ''}`}>
                      Đang giao hàng
                    </div>
              ...
              },
              {
                ...
                children: (
                  <div className='w-[30rem]'>
                    <div className={`text-lg font-semibold ${count == 1 ? 'text-green-500' : ''}`}>
              ...
              },
              {
                ...
                children: (
                  <div className='w-[30rem]'>
                    <div className={`text-lg font-semibold ${count == 2 ? 'text-green-500' : ''}`}>
                      Đơn hàng đã rời kho phân loại
                    </div>
                    <span className='text-sm text-gray-500'>
                      {gio || ''}, {thu || ''} {ngay}
...

              {
                ...
                children: (
                  <div className='w-[30rem]'>
                    <div className={`text-lg font-semibold ${count == 3 ? 'text-green-500' : ''}`}>
...

              {
                ...
                children: (
                  <div className='w-[30rem]'>
                    <div className={`text-lg font-semibold ${count == 4 ? 'text-green-500' : ''}`}>
                      Đang được chuẩn bị
                    </div>
                    <span className='text-sm text-gray-500'>
                      {gio || ''}, {thu || ''} {ngay}
...

              {
                ...
                children: (
                  <div className='w-[30rem]'>
                    <div className={`text-lg font-semibold ${count == 5 ? 'text-green-500' : ''}`}>
                      Đặt hàng thành công
                    </div>
                    <span className='text-sm text-gray-500'>
                      {gio || ''}, {thu || ''} {ngay}
